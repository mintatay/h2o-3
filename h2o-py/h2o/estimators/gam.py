...
class H2OGeneralizedAdditiveEstimator(H2OEstimator):
    ...
    param_names = {"model_id", "training_frame", "validation_frame", "nfolds", "seed", "keep_cross_validation_models",
                   ...
                   "max_runtime_secs", "custom_metric_func", "k", "knots_keys", "gam_X", "bs", "scale", "saveZMatrix",
                   "savePenaltyMat", "saveGamCols"}
    ...
    @property
    def saveZMatrix(self):
        """
        Save Ztranspose used to centering

        Type: ``bool``  (default: ``False``).
        """
        return self._parms.get("saveZMatrix")

    @saveZMatrix.setter
    def saveZMatrix(self, saveZMatrix):
        assert_is_type(saveZMatrix, None, bool)
        self._parms["saveZMatrix"] = saveZMatrix


    @property
    def savePenaltyMat(self):
        """
        Save penalty matrix

        Type: ``bool``  (default: ``False``).
        """
        return self._parms.get("savePenaltyMat")

    @savePenaltyMat.setter
    def savePenaltyMat(self, savePenaltyMat):
        assert_is_type(savePenaltyMat, None, bool)
        self._parms["savePenaltyMat"] = savePenaltyMat
